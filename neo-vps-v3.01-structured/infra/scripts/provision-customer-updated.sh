...
# ===================================
# This script automatically provisions a complete Multi-Panel hosting
...
Automatically provision Multi-Panel hosting infrastructure for a new customer.
...
variable "panel_hostname" {
  description = "Multi-Panel hostname"
  type        = string
  default     = "panel.$DOMAIN"
}
...
output "server_public_ip" {
  ...
  value       = module.panel_server.public_ip
}
...
output "access_urls" {
  ...
  value = {
    whm     = "https://\${module.panel_server.public_ip}:2087"
    panel  = "https://\${module.panel_server.public_ip}:2083"
    webmail = "https://\${module.panel_server.public_ip}:2096"
  }
}
...
output "next_steps" {
  ...
  1. Update DNS records for ${DOMAIN}:
     - Point ${DOMAIN} A record to \${module.panel_server.public_ip}
     - Create ns1.${DOMAIN} A record pointing to \${module.panel_server.public_ip}
     - Create ns2.${DOMAIN} A record pointing to \${module.panel_server.public_ip}
  
  2. SSH to server and install Multi-Panel:
     ssh root@\${module.panel_server.public_ip}
     cd /home && curl -o latest -L https://securedownloads.panel.net/latest
     sh latest
  
  3. Access WHM: https://\${module.panel_server.public_ip}:2087
  
  4. Configure:
     - Enter Multi-Panel license
     - Set up nameservers
